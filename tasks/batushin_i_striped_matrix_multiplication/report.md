...
- `ComputeBlockSizes()` — вычисление количества элементов на процесс
- `ComputeBlockOffsets()` — вычисление смещений для MPI операций
- `RunSequentialFallback()` — последовательная реализация с рассылкой результата
- `RunStripedScheme()` — точка входа в ленточную схему
- `DistributeMatrixA()` — распределение строк матрицы A через `MPI_Scatterv`
- `DistributeMatrixB()` — начальная рассылка вертикальных полос матрицы B
- `ComputeWithCyclicShift()` — реализация циклического сдвига и локальных вычислений
- `BroadcastFinalResult()` — сбор результатов через `MPI_Gatherv` и рассылка через `MPI_Bcast`
